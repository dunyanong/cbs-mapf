  ...
  int AstarSTGrid2d::_search() {
    ...
    if (_open.empty()) {
        std::cout << "[WARNING] Open list empty! No path found." << std::endl;
    }
  ...
  bool AstarSTGrid2d::_check_terminate(StateST& s) {
    std::cout << "[DEBUG] Checking termination for state: " << s << std::endl;
    if (s.v == _vg) {
        std::cout << "[DEBUG] GOAL REACHED at state: " << s << std::endl;
        return true;
    }
  ...
  bool AstarSTGrid2d::_collide_check(long v1, long v2, long t) {
    if (v2 == _vg) {
        std::cout << "[DEBUG] Checking constraints for GOAL vertex: " << v2 << " at t=" << t << std::endl;
    }    
